...
const Item =({item, onPress, backgroundColor, borderColor, color}) => {
    return(
    <Pressable style={[Register.chips, {backgroundColor: backgroundColor, borderColor: borderColor}]} onPress={onPress}>
            <View >
         ...
         <Modal
          ...
          >
                        ...
                        <View style={style.modalBody} >
                            {
                                REASONS.map((reason) => {
                                    const backgroundColor = reason.key === selectReason ? COLORS.darkPink : COLORS.white;
                                    const borderColor = reason.key === selectReason ? COLORS.darkPink : COLORS.grey;  
                                    const color = reason.key === selectReason ? COLORS.white : COLORS.grey;  
                                    return(
                                        <React.Fragment key={reason.key} >
                                            <Item item={reason.dahilan} 
                                                    borderColor={borderColor} 
                                                    backgroundColor={backgroundColor} 
                                                    color={color} 
                                                    onPress={()=>handleReason(reason.key,reason.dahilan)}   
                                                    />
                                        </React.Fragment>
                                    )
                                })
