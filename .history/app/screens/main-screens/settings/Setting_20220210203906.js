...
const ItemSettings = ({onPress, label, color}) => {
    return(
      <View style={styling.settingItemWrapper}  > 
        <TouchableOpacity onPress={onPress} >
          <Text style={[styling.settingItemText, {color: color}]}  >{label}</Text>
        </TouchableOpacity>
      </View>
    )
}
     ...
     <HeaderWrapper />
      <SafeAreaView style={{flex: 1,  backgroundColor: COLORS.white}}  >
        <View style={{flex: 1}}>
                ...
                <View>
                     <ItemSettings  label="Logout" onPress={()=>signOut()}  />
                     <ItemSettings  label="Delete Account"  />
                </View>
