...
export default function SignUpPage({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!data.email.match(emailRegex)) {
      ...
      if (userExisting) {
        ...
        try {
          setMessage(
            "Please standby while we are processing your PaperStack OTP."
          );
          ...
          if (createGenerator) {
            ...
            const allowGenerator = await client.allowGenerateOTP(data.email);
            ...
            navigation.navigate("OTPPaperStack");
          } else {
  ...
  useEffect(async () => {
    const fetchUsers = async () => {
      ...
      let user;
      try {
        user = await axios.post(`${BACKEND_BASEURL}/api/registers/`, {
          email,
        });
      } catch (err) {
        console.log(err);
      ...
      if (user && user.data.user) return setUserExisting(true);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View>
          ...
          <View style={AuthenticationStyle.textWrapper}>
            <Text style={AuthenticationStyle.textText}>
          ...
          <View style={AuthenticationStyle.noAccountWrap}>
            <Text style={AuthenticationStyle.noAccountText}>
