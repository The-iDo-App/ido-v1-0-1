...
export default function QuestionInterest() {
  ...
  useEffect(async () => {
    if (isSubmitButtonClicked) {
      // console.log(currentQuestionIndex, currentOptionSelected);
      const userId = await AsyncStorage.getItem('userId');
      const interest = await AsyncStorage.getItem('interestId');
      try {
        const response = await axios.post(
          `${BACKEND_BASEURL}/api/evaluations`,
          {
            userId,
            interest,
            questions,
        ...
        {questionsDB[currentQuestionIndex]?.choices.map((choice) => (
          ...
          >
            <Text
              style={{
                color:
                  choice === currentOptionSelected ? COLORS.white : COLORS.grey,
                fontSize: 16,
                padding: 10 / scale,
              }}
            >
    ...
    if (showNextButton) {
      return (
        <View>
          <NextButton
            TextButton={
              currentQuestionIndex === questionsDB.length - 1
                ? 'Submit'
                : 'Next'
            }
  ...
  return (
    ...
    >
      ...
      <ScrollView
        style={{ height: height / 1.8, backgroundColor: COLORS.white }}
      >
        {/*Question*/}
        {renderQuestion()}
